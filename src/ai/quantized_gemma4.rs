...
impl ModelWeights {
    pub fn from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        // Gemma 4 has separate dims for global vs sliding window attention
        ...
        for layer_idx in 0..block_count {
            ...
            // Detect global vs sliding window from actual Q weight shape
            let q_out_dim = attention_wq.shape().dims()[0];
            let layer_head_dim = q_out_dim / head_count;
            let is_sliding = layer_head_dim == key_length_swa;
            let sliding_window_size = is_sliding.then_some(sliding_window_size);
            let layer_q_dim = head_count * layer_head_dim;
    ...
    pub fn forward(&mut self, x: &Tensor, index_pos: usize) -> Result<Tensor> {
        ...
        for layer in self.layers.iter_mut() {
            ...
            let mut hidden = (x + residual)?;

            // 2. Feed-forward block
            let _enter = layer.span_mlp.enter();
            let residual = &hidden;
            let x = layer.ffn_norm.forward(&hidden)?;
            ...
            if let Some(ref scale) = layer.layer_output_scale {
                let scale = scale.broadcast_as(hidden.shape())?;
                hidden = (hidden * scale)?;
            }
